...
# cache path baked into the widget's KConfig, installs the plasmoid, wires the
# exporter daemon up (systemd user unit or XDG autostart), initializes the
# SQLite DB, and links the commands into ~/.local/bin.
#
# Usage:
...
usage() {
    awk 'NR==2{f=1} f{if ($0 !~ /^#/) exit; sub(/^# ?/, ""); print}' "${BASH_SOURCE[0]}"
}
...
if [ "$PURGE" -eq 1 ] && [ "$DO_UNINSTALL" -eq 0 ]; then
    echo "note: --purge has no effect without --uninstall" >&2
fi

PREFIX="$(readlink -f "$PREFIX" 2>/dev/null || echo "$PREFIX")"

# Refuse prefixes whose deletion would be catastrophic (rm -rf target).
prefix_is_safe() {
    local p="$1"
    [ -n "$p" ] || return 1
    [ "$p" = "/" ] && return 1
    [ "$p" = "$HOME" ] && return 1
    [ "$p" = "$SCRIPT_DIR" ] && return 1
    case "$SCRIPT_DIR" in "$p"|"$p"/*) return 1 ;; esac   # never delete our own tree
    local depth
    depth="$(awk -F'/' '{print NF - 1}' <<< "$p")"
    [ "$depth" -ge 3 ] || return 1                        # no /usr-style shallow targets
    return 0
}
...
find_old_paths() {
    # The only machine-specific path baked into the repo is the widget's
    # stateFile default in main.xml; the QML resolves rehoboam_config.py
    # relatively, so it needs no patching.
    ...
    OLD_CACHE="${OLD_CACHE:-}"
    if [ -n "$OLD_CACHE" ] && [ "$OLD_CACHE" != "$CACHE_DIR" ]; then
        echo "  [INFO] patching references to $OLD_CACHE -> $CACHE_DIR"
    fi
}

copy_project() {
    echo "[2/6] Copying project to $PREFIX"
    if ! prefix_is_safe "$PREFIX"; then
        echo "  [ERROR] refusing unsafe install prefix: $PREFIX" >&2
        echo "          must be at least 3 path levels deep, not \$HOME, not '/', and" >&2
        echo "          must not contain this script's own tree" >&2
        exit 1
    fi
    rm -rf "$PREFIX"
    mkdir -p "$PREFIX"
...
patch_widget() {
    echo "[3/6] Patching widget paths and installing plasmoid"
    local -a targets=(
    ...
    for f in "${targets[@]}"; do
        [ -n "$OLD_CACHE" ] && [ "$OLD_CACHE" != "$CACHE_DIR" ] &&
            sed -i "s|$OLD_CACHE|$CACHE_DIR|g" "$f"
    done
...
init_db() {
    ...
    local out status=0
    out="$(PREFIX="$PREFIX" python3 - <<'PYEOF'
import os, sys
sys.path.insert(0, os.environ["PREFIX"])
import rehoboam_db
...
make_links() {
    ...
    echo "[5/6] Linking commands into $BIN_DIR"
    mkdir -p "$BIN_DIR"
...
setup_autostart() {
    ...
    echo "[6/6] Wiring up the exporter daemon"
    EXPORTER_START_TS="$(date +%s)"
...
verify_statefile() {
    if [ "$DO_AUTOSTART" -eq 0 ]; then
        echo "  [INFO] exporter autostart disabled — launch it manually to feed the widget"
        return 0
    fi
    sleep 2
    local state="$CACHE_DIR/rehoboam_widget.json" mtime=0
    [ -f "$state" ] && mtime="$(stat -c %Y "$state" 2>/dev/null || echo 0)"
    if [ "${mtime:-0}" -gt "${EXPORTER_START_TS:-0}" ]; then
        echo "  [OK] fresh widget state written: $state"
    else
        echo "  [WARN] no fresh snapshot yet — check: journalctl --user -u rehoboam-exporter"
    fi
}
...
uninstall() {
    ...
    if ! prefix_is_safe "$installed"; then
        echo "  [ERROR] refusing to delete unsafe prefix: $installed — remove it manually if you are sure" >&2
        installed=""
    fi
    systemctl --user disable --now rehoboam-exporter.service >/dev/null 2>&1 || true
    pkill -f "rehoboam_exporter\.py" >/dev/null 2>&1 || true   # nohup/autostart instance
    rm -f "$unit"
    rm -f "$desk"
    local -a bin_names=(rehoboam rehoboam.sh kanban kanban.sh timew timew.sh \
                        rehoboam-config rehoboam-exporter rehoboam-reload)
    local name resolved
    for name in "${bin_names[@]}"; do
        [ -L "$BIN_DIR/$name" ] || continue                     # never touch real files
        resolved="$(readlink -f "$BIN_DIR/$name")"
        case "$resolved" in
            "$installed"/*) rm -f "$BIN_DIR/$name" ;;
            *) echo "  [INFO] kept $BIN_DIR/$name (points outside the install: $resolved)" ;;
        esac
    done
    rm -rf "$PLASMOID_DIR"
    if [ -n "$installed" ]; then
